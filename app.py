...
def load_app_config():
    """Load configuration for Streamlit app"""
...
def init_session_state():
    ...
    if 'brsf_scale' not in st.session_state:
        streamlit_config = APP_CONFIG.get('streamlit_settings', {})
        st.session_state.brsf_scale = streamlit_config.get('brsf_scale_default')
...
def get_document_type(result: Dict[str, Any]) -> str:
    ...
    if metadata.get('is_consolidated_format', False):
        return "Consolidated"
    
    # The parser already detected if this is abbreviated format from PDF indicators
    is_abbreviated = metadata.get('is_abbreviated_format', False)
    
    if is_abbreviated:
        # Check if it was enriched from Nota Integrativa
        ...
        # Check for orphan voices (also indicates NI enrichment)
        ...
        # Pure abbreviated without NI details
        return "Abbreviated"
    
    return "Extended"

def get_document_year(result: Dict[str, Any]) -> str:
    """Get document year from full date"""
    if not result:
        return "N/A"
    
    full_date = result.get('metadata', {}).get('anno_bilancio', 'N/A')
    if full_date == 'N/A':
        return 'N/A'
    
    # Extract year from full date (DD/MM/YYYY format)
    try:
        if '/' in full_date:
...
def recalculate_brsf(modified_result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Recalculate totals with special rules using DataValidator
        validator_config = {'validation_settings': {
            'tolerance_percentage': 0.01,
            'tolerance_minimum': 2.0,
            'immediate_success_threshold': 2.0
...
def main():
    ...
    # Validate file count
    streamlit_config = APP_CONFIG.get('streamlit_settings', {})
    max_files = streamlit_config.get('max_files', 5)

    if uploaded_files and len(uploaded_files) > max_files:
        st.error(f"❌ Maximum {max_files} files allowed. Please select fewer files.")
