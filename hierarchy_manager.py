# hierarchy_manager.py - FIXED VERSION
...
class HierarchyContext:
    ...
    parent_stack: List[Dict[str, Any]]  # Changed back to Dict for consistency
...
class HierarchyManager:
    ...
    This module represents the most complex and critical component of the financial
    statement parsing system. It handles the sophisticated parent-child relationship
    ...
    The parent stack mechanism is the heart of this system. It tracks the current
    ...
    def process_financial_item(self, financial_item: Dict[str, Any], 
        ...
        This method implements the core parent stack management logic from the
        original parser's _extract_flat_data method. It represents the most
        critical functionality of the entire parsing system, as any errors in
        parent stack management result in incorrect hierarchical structures
        and consequently invalid financial statement representations.
        
        The method handles the complex decision-making process for each financial
        ...
        # This logic is preserved from the original parser for consistency. It ensures
        # that a potential parent with a zero value from the PDF is marked to be
    ...
    def handle_abbreviated_scadenze(self, item_data: Dict[str, Any], 
        ...
        if maturity_key and line_value != 0.0:
            ...
            if logging.getLogger().isEnabledFor(logging.DEBUG):
                logging.debug(
                    f"    -> ABBREVIATED FORMAT: Associated '{maturity_key}' with parent "
    ...
    def build_hierarchical_structure(self, config_node: Dict[str, Any], 
        ...
        This method preserves the exact _build_hierarchical_structure logic
        from the original parser. It transforms the flat data map produced
        by the parsing process into the final nested dictionary structure
        that represents the complete financial statement hierarchy.
